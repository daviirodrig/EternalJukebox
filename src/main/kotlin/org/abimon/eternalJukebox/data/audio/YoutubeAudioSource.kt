...
object YoutubeAudioSource : IAudioSource {
    ...
    override suspend fun provide(info: JukeboxInfo, clientInfo: ClientInfo?): DataSource? {
        ...
        if (apiKey == null) {
            ...
            val artistTitle = getMultiContentDetailsWithKey(searchYoutubeWithKey(
                "${info.artist} - ${info.title}", 10
            ).map { it.id.videoId })

            val x = artistTitle.firstOrNull() ?: return kotlin.run {
                logger.error(
    ...
    override suspend fun provideLocation(info: JukeboxInfo, clientInfo: ClientInfo?): URL? {
        ...
        val artistTitleLyrics = getMultiContentDetailsWithKey(searchYoutubeWithKey(
            "${info.artist} - ${info.title} lyrics", 10
        ).map { it.id.videoId })
